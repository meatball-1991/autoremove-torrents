...
class Strategy(object):
    def __init__(self, name, conf):
        ...
        self._all_categories = (
            conf["all_categories"]
            if "all_categories" in conf
            else not "categories" in conf
        )
        self._all_trackers = (
            conf["all_trackers"] if "all_trackers" in conf else not "trackers" in conf
        )
        self._all_status = (
            conf["all_status"] if "all_status" in conf else not "status" in conf
        )

        # Print debug log
        self._logger.debug("Configuration of strategy '%s':" % self._name)
        self._logger.debug(
            "Configurated filters and conditions: %s" % ", ".join(self._conf)
        )
    ...
    def _apply_filters(self):
        filter_conf = [
            {
                "all": self._all_categories,
                "ac": "categories",
                "re": "excluded_categories",
            },  # Category filter
            {
                "all": self._all_status,
                "ac": "status",
                "re": "excluded_status",
            },  # Status filter
            {
                "all": self._all_trackers,
                "ac": "trackers",
                "re": "excluded_trackers",
            },  # Tracker filter
        ]
        ...
        for i in range(0, len(filter_conf)):
            ...
            self._logger.debug(
                "Filter configrations: ALL: %s; ACCEPTANCES: [%s]; REJECTIONS: [%s]."
                % (
                    filter_conf[i]["all"],
            ...
            self._logger.debug(
                "INPUT: %d torrent(s) before applying the filter."
            ...
            self._logger.debug(
                "OUTPUT: %d torrent(s) after applying the filter."
                % len(self.remain_list)
            )
            for torrent in self.remain_list:
                self._logger.debug(torrent)
    ...
    def _apply_conditions(self, client_status):
        ...
        for conf in self._conf:
            if conf in conditions:
                # Print debug log
                self._logger.debug(
                    "Applying condition %s..." % conditions[conf].__name__
                )
                self._logger.debug(
                    "INPUT: %d torrent(s) to be reserved before applying the condition."
                    % len(self.remain_list)
                )
                for torrent in self.remain_list:
                    self._logger.debug(torrent)
                ...
                self._logger.debug(
                    "OUTPUT: %d torrent(s) to be reserved after applying the condition."
                    % len(self.remain_list)
                )
                for torrent in self.remain_list:
                    self._logger.debug(torrent)
                self._logger.debug(
                    "OUTPUT: %d torrent(s) to be removed after applying the condition."
                    % len(self.remove_list)
                )
    ...
    def execute(self, client_status, torrents):
        self._logger.debug("Running strategy %s..." % self._name)
