...
class qBittorrent(object):
    ...
    class qBittorrentAPIHandlerV1(object):
        ...
        def check_compatibility(self):
            request = self._session.get(self._host + "/version/api", verify=False)
        ...
        def api_version(self):
            return self._session.get(self._host + "/version/api", verify=False)

        # Get client version
        def client_version(self):
            return self._session.get(self._host + "/version/qbittorrent", verify=False)

        # Login
        def login(self, username, password):
            return self._session.post(
                self._host + "/login", data={"username": username, "password": password}
            )

        # Get server state
        def server_state(self):
            return self._session.get(self._host + "/sync/maindata", verify=False)

        # Get torrent list
        def torrent_list(self):
            return self._session.get(self._host + "/query/torrents", verify=False)

        # Get torrent's generic properties
        def torrent_generic_properties(self, torrent_hash):
            return self._session.get(
                self._host + "/query/propertiesGeneral/" + torrent_hash, verify=False
            )

        # Get torrent's tracker
        def torrent_trackers(self, torrent_hash):
            return self._session.get(
                self._host + "/query/propertiesTrackers/" + torrent_hash, verify=False
            )

        # Batch Delete torrents
        def delete_torrents(self, torrent_hash_list):
            return self._session.post(
                self._host + "/command/delete",
                data={"hashes": "|".join(torrent_hash_list)},
            )

        # Batch Delete torrents and data
        def delete_torrents_and_data(self, torrent_hash_list):
            return self._session.post(
                self._host + "/command/deletePerm",
                data={"hashes": "|".join(torrent_hash_list)},
            )
    ...
    class qBittorrentAPIHandlerV2(object):
        ...
        def check_compatibility(self):
            request = self._session.get(
                self._host + "/api/v2/app/webapiVersion", verify=False
            )
            return request.status_code != 404  # compatible if API exsits
        ...
        # Get client version
        def client_version(self):
            return self._session.get(self._host + "/api/v2/app/version", verify=False)
        ...
        # Get server state
        def server_state(self):
            return self._session.get(self._host + "/api/v2/sync/maindata", verify=False)

        # Get torrent list
        def torrent_list(self):
            return self._session.get(self._host + "/api/v2/torrents/info", verify=False)
        ...
        def torrent_trackers(self, torrent_hash):
            return self._session.get(
                self._host + "/api/v2/torrents/trackers",
                verify=False,
                params={"hash": torrent_hash},
            )
        ...
        def delete_torrents(self, torrent_hash_list):
            return self._session.get(
                self._host + "/api/v2/torrents/delete",
                verify=False,
    ...
    def __init__(self, host):
        ...
        for obj in [
            self.qBittorrentAPIHandlerV2,
            self.qBittorrentAPIHandlerV1,
        ]:  # New version API first
            handler = obj(host)
        ...
        if self._request_handler is None:
            raise IncompatibleAPIVersion(
                "Incompatible qbittorrent client. The current API version may be unsupported."
            )
    ...
    def api_version(self):
        return "%s (%s)" % (
            self._request_handler.api_version().text,
            self._request_handler.api_major_version(),
        )
    ...
    def torrent_properties(self, torrent_hash):
        ...
        for torrent in self._torrents_list_cache:
            if torrent["hash"] == torrent_hash:
                ...
                properties = self._request_handler.torrent_generic_properties(
                    torrent_hash
                ).json()
                ...
                if "category" in torrent:
                    torrent_obj.category = (
                        [torrent["category"]] if len(torrent["category"]) > 0 else []
                    )
                elif "label" in torrent:
                    torrent_obj.category = (
                        [torrent["label"]] if len(torrent["label"]) > 0 else []
                    )
    ...
    def remove_torrents(self, torrent_hash_list, remove_data):
        request = (
            self._request_handler.delete_torrents_and_data(torrent_hash_list)
            if remove_data
